...
public class TestConvertor {
    public static void main(String[] args) {
    	...
    	String mainDirectoryPath = System.getProperty("katalonDir");
        String objectFilePath = Paths.get(inputDirectoryPath, "src/test/java/Pages/").toString();
        String inputFilePath = Paths.get(inputDirectoryPath, "src", "test", "java", "StepDefinitions").toString();
        File testDir = new File(inputFilePath);
        File[] testFiles = testDir.listFiles((d, name) -> name.endsWith(".java"));
        
        File objDir = new File(objectFilePath);
        File[] objectFiles = objDir.listFiles((d, name) -> name.endsWith(".java"));
        
        if (testFiles != null && objectFiles != null) {
            int minLength = Math.min(testFiles.length, objectFiles.length); // Ensure we don't exceed array bounds

            for (int i = 0; i < minLength; i++) {
                File inputFile = testFiles[i];
                File objectFile = objectFiles[i];
            try {
                String className = inputFile.getName().replace(".java", "");
                System.out.println("Processing Test File: " + inputFile.getName());
                
                String seleniumCode = readFromFile(inputFile.getAbsolutePath());
                
               // File objectFile = new File(objectFilePath, className + ".java");
                Map<String, String> objectMap = readObjectFile(objectFile.getAbsolutePath());
